...
// Tiny markdown-ish renderer for the subset used in data/blog.js: headings,
// **bold**, *italic*, "- " bullets, and blank-line paragraphs.
const renderContent = (markdown) => {
  const html = markdown
    .replace(/^#### (.*)$/gm, '<h4 class="text-xl font-bold text-gray-100 mb-4 mt-8">$1</h4>')
    .replace(/^### (.*)$/gm, '<h3 class="text-2xl font-bold text-gray-100 mb-4 mt-8">$1</h3>')
    .replace(/^## (.*)$/gm, '<h2 class="text-3xl font-bold text-gray-100 mb-4 mt-8">$1</h2>')
    .replace(/^# (.*)$/gm, '<h1 class="text-4xl font-bold text-gray-100 mb-4 mt-8">$1</h1>')
    .replace(/\*\*(.*?)\*\*/g, '<strong class="text-white">$1</strong>')
    .replace(/\*(.*?)\*/g, '<em class="text-green-400">$1</em>')
    .replace(/^- (.*)$/gm, '<li class="ml-6">$1</li>')
    .split(/\n\n+/)
    .map((block) =>
      /^<h[1-4]|^<li/.test(block.trim()) ? block : `<p class="mb-4">${block}</p>`
    )
    .join('\n');
  return html;
};
...
const BlogDetail = ({ blog, goBack }) => {
  ...
  const relatedPosts = blogData.filter((b) => b.id !== blog.id && !b.draft).slice(0, 3);
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-100 mb-6 leading-tight">{blog.title}</h1>
          ...
          <div className="relative mb-12">
            <img src={blog.image} alt={blog.title} className="w-full h-64 md:h-80 object-cover rounded-2xl shadow-2xl" />
            ...
            <div className="flex items-start space-x-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-300 mb-4">
                  Data Scientist &amp; Spatial Analyst passionate about geospatial analysis,
                  machine learning, and creating data-driven stories that make a difference.
                </p>
                <div className="flex space-x-4">
                  <a href="https://github.com/bxjyj" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-green-400 transition-colors duration-300">
                    <i className="fab fa-github text-xl"></i>
                  </a>
                  <a href="https://www.linkedin.com/in/jiayi-jade-xu/" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-green-400 transition-colors duration-300">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {relatedPosts.map((relatedBlog) => (
                <div
                  key={relatedBlog.id}
                  className="bg-gray-700/50 rounded-xl overflow-hidden border border-gray-600 hover:border-green-500/50 transition-all duration-300 group cursor-pointer"
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed mb-4">{relatedBlog.excerpt}</p>
